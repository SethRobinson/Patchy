...
Rect layer_render_bounds(const Layer& layer, const std::vector<LayerBoundsOverride>* overrides) {
...
void composite_color(QImage& destination, std::int32_t x, std::int32_t y, RgbColor color, float alpha, BlendMode mode,
                     bool preserve_alpha, std::int32_t origin_x = 0, std::int32_t origin_y = 0) {
  alpha = clamp_unit(alpha);
  const auto image_x = x - origin_x;
  const auto image_y = y - origin_y;
  if (alpha <= 0.0F || image_x < 0 || image_y < 0 || image_x >= destination.width() ||
      image_y >= destination.height()) {
    return;
  }

  auto* dst = destination.scanLine(image_y) + static_cast<std::size_t>(image_x) * (preserve_alpha ? 4U : 3U);
  const auto da = preserve_alpha ? static_cast<float>(dst[3]) / 255.0F : 1.0F;
  const auto out_a = preserve_alpha ? (alpha + da * (1.0F - alpha)) : 1.0F;
...
void render_drop_shadow(QImage& destination, const Layer& layer, Rect clip, Rect bounds, const LayerDropShadow& shadow,
                        bool preserve_alpha, std::int32_t origin_x, std::int32_t origin_y) {
  ...
  for (std::int32_t y = draw_rect.y; y < draw_rect.y + draw_rect.height; ++y) {
    for (std::int32_t x = draw_rect.x; x < draw_rect.x + draw_rect.width; ++x) {
      ...
      composite_color(destination, x, y, shadow.color, alpha, shadow.blend_mode, preserve_alpha, origin_x, origin_y);
    }
  }
}

void render_outer_glow(QImage& destination, const Layer& layer, Rect clip, Rect bounds, const LayerOuterGlow& glow,
                       bool preserve_alpha, std::int32_t origin_x, std::int32_t origin_y) {
  ...
  for (std::int32_t y = draw_rect.y; y < draw_rect.y + draw_rect.height; ++y) {
    for (std::int32_t x = draw_rect.x; x < draw_rect.x + draw_rect.width; ++x) {
      ...
      composite_color(destination, x, y, glow.color, glow_alpha, glow.blend_mode, preserve_alpha, origin_x, origin_y);
    }
  }
}

void render_gradient_fill(QImage& destination, const Layer& layer, Rect clip, Rect bounds, const LayerGradientFill& fill,
                          bool preserve_alpha, std::int32_t origin_x, std::int32_t origin_y) {
  ...
  for (std::int32_t y = draw_rect.y; y < draw_rect.y + draw_rect.height; ++y) {
    for (std::int32_t x = draw_rect.x; x < draw_rect.x + draw_rect.width; ++x) {
      ...
      composite_color(destination, x, y, gradient_color(fill.gradient, position), alpha, fill.blend_mode,
                      preserve_alpha, origin_x, origin_y);
    }
  }
}

void render_bevel_emboss(QImage& destination, const Layer& layer, Rect clip, Rect bounds, const LayerBevelEmboss& bevel,
                         bool preserve_alpha, std::int32_t origin_x, std::int32_t origin_y) {
    ...
    for (std::int32_t x = draw_rect.x; x < draw_rect.x + draw_rect.width; ++x) {
      ...
      if (lighting > 0.0F) {
        composite_color(destination, x, y, bevel.highlight_color,
                        ...
                        bevel.highlight_blend_mode, preserve_alpha, origin_x, origin_y);
        ...
        composite_color(destination, x, y, bevel.shadow_color,
                        ...
                        bevel.shadow_blend_mode, preserve_alpha, origin_x, origin_y);
      }
    }
  }
}
...
void render_stroke(QImage& destination, const Layer& layer, Rect clip, Rect bounds, const LayerStroke& stroke,
                   bool preserve_alpha, std::int32_t origin_x, std::int32_t origin_y) {
  ...
  for (std::int32_t y = draw_rect.y; y < draw_rect.y + draw_rect.height; ++y) {
    for (std::int32_t x = draw_rect.x; x < draw_rect.x + draw_rect.width; ++x) {
      ...
      composite_color(destination, x, y, color, alpha, stroke.blend_mode, preserve_alpha, origin_x, origin_y);
    }
  }
}
...
void composite_layer(QImage& destination, const Layer& layer, bool preserve_alpha, Rect clip, std::int32_t origin_x,
                     std::int32_t origin_y, const std::vector<LayerBoundsOverride>* overrides);

void composite_pixel_layer(QImage& destination, const Layer& layer, bool preserve_alpha, Rect clip,
  ...
  const auto bounds = layer_render_bounds(layer, overrides);
  const auto& style = layer.layer_style();
  if (style.effects_visible) {
    for (const auto& shadow : style.drop_shadows) {
      render_drop_shadow(destination, layer, clip, bounds, shadow, preserve_alpha, origin_x, origin_y);
    }
    for (const auto& glow : style.outer_glows) {
      render_outer_glow(destination, layer, clip, bounds, glow, preserve_alpha, origin_x, origin_y);
    }
  }
  ...
  if (!draw_rect.empty()) {
    ...
    for (int y = draw_rect.y; y < draw_rect.y + draw_rect.height; ++y) {
      auto* destination_row = destination.scanLine(y - origin_y);
      ...
      for (int x = draw_rect.x; x < draw_rect.x + draw_rect.width; ++x) {
        ...
        auto* dst = destination_row + static_cast<std::size_t>(x - origin_x) * (preserve_alpha ? 4U : 3U);
        const auto da = preserve_alpha ? static_cast<float>(dst[3]) / 255.0F : 1.0F;
        ...
        for (int channel = 0; channel < 3; ++channel) {
          if (preserve_alpha && out_a > 0.0F) {
            out_channels[channel] =
                (static_cast<float>(blended[static_cast<std::size_t>(channel)]) * sa +
                 dst_channels[channel] * da * (1.0F - sa)) /
                out_a;
          } else {
            out_channels[channel] =
                static_cast<float>(blended[static_cast<std::size_t>(channel)]) * sa +
                dst_channels[channel] * (1.0F - sa);
          }
        }
  ...
  if (style.effects_visible) {
    for (const auto& fill : style.gradient_fills) {
      render_gradient_fill(destination, layer, clip, bounds, fill, preserve_alpha, origin_x, origin_y);
    }
    for (const auto& bevel : style.bevels) {
      render_bevel_emboss(destination, layer, clip, bounds, bevel, preserve_alpha, origin_x, origin_y);
    }
    for (const auto& stroke : style.strokes) {
      render_stroke(destination, layer, clip, bounds, stroke, preserve_alpha, origin_x, origin_y);
    }
  }
}
...
void composite_layer(QImage& destination, const Layer& layer, bool preserve_alpha, Rect clip, std::int32_t origin_x,
  ...
  if (layer.kind() == LayerKind::Group) {
    for (const auto& child : layer.children()) {
      composite_layer(destination, child, preserve_alpha, clip, origin_x, origin_y, overrides);
    }
    return;
  }
  composite_pixel_layer(destination, layer, preserve_alpha, clip, origin_x, origin_y, overrides);
}
...
QImage qimage_from_document(const Document& document, bool preserve_alpha) {
  ...
  for (const auto& layer : document.layers()) {
    composite_layer(image, layer, preserve_alpha, canvas, 0, 0, nullptr);
  }
  return image;
}
...
QImage render_document_rect(const Document& document, QRect document_rect, bool preserve_alpha,
  ...
  for (const auto& layer : document.layers()) {
    composite_layer(image, layer, preserve_alpha, clip, clip.x, clip.y, overrides);
  }
  return image;
}
...
QImage qimage_from_document_rect_with_layer_bounds(
  ...
  std::vector<LayerBoundsOverride> overrides;
  overrides.reserve(layer_bounds.size());
  for (const auto& [layer_id, bounds] : layer_bounds) {
    overrides.push_back(LayerBoundsOverride{layer_id, bounds});
  }
