...
void MainWindow::bind_action_translations(ActionBuildContext& ctx) {
  ...
  const std::vector<std::pair<QAction*, const char*>> translated_actions = {
      ...
      {ctx.layer_new_menu->menuAction(), "&New"},
      ...
      {ctx.layer_mask_menu->menuAction(), "Layer Mask"},
      {ctx.vector_mask_menu->menuAction(), "&Vector Mask"},
      {ctx.layer_smart_objects_menu->menuAction(), "Smart Objects"},
      {ctx.layer_arrange_menu->menuAction(), "Arran&ge"},
      ...
      {view_layer_mask_action_, "View Layer Mask"},
      ...
      {duplicate_layer_to_document_action_, "Duplicate Layer to Document..."},
      ...
      {ctx.rotate_cw_action, "Rotate &Right"},
      {ctx.rotate_ccw_action, "Rotate &Left"},
      {ctx.rotate_arbitrary_action, "Rotate &Arbitrary..."},
  ...
  const std::vector<std::pair<QObject*, const char*>> translated_widgets = {
      ...
      {mixer_sample_all_layers_check_, "Sample All Layers"},
