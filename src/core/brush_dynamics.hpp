...
// Which per-stroke input drives a dynamic parameter (Photoshop's "Control" dropdown). v1 wires
// the full set for the tip angle only; size/roundness/scatter/count import their jitter without
// a control, and the global pen-pressure preferences stay authoritative for size/opacity.
...
struct BrushDynamics {
  ...
  std::uint32_t seed{0};
  double pen_control_value{1.0};  // pen pressure 0..1 (PenPressure angle control)
  double pen_angle_degrees{0.0};  // tilt azimuth or barrel rotation (PenTilt / PenRotation)
  bool pen_angle_valid{false};
...
[[nodiscard]] int sample_dab_count(const BrushDynamics& dynamics, BrushDynamicsRng& rng) noexcept;
