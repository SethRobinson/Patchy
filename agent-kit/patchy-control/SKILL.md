...
For art from a supplied photo or image, read [Reference artwork](references/reference-art.md), also available as `get_help` with `topic: "reference-art"`. Use it to plan the crop, palette, editable layers, and preview comparisons. The setup guide includes example user requests for icons, sprite processing, and PSD edits.
