...
- Visible automation has Pause/Resume beside Stop and Slow. Respect the user's pause; it preserves the running script and its Undo grouping. Window movement, resizing, zoom and pan remain usable during normal, Slow and paused playback. `patchy.ui.paused` shares this visible MCP/CLI control, resets at run end, and cannot be enabled headlessly. A paused request stays busy; the user resumes it with the window button. Do not submit another editing request to bypass a pause.
