...
- Tools sharing a slot go through the shared `create_flyout_tool_action` + `configure_tool_flyout` lambdas (nine flyouts: marquee, lasso, wand, gradient+fill, stamp, smudge/detail, dodge/tone, shape [Line/Rect/Ellipse/Polygon/Custom Shape, default Rect], path tools [Path Select A / Direct Select Shift+A]). The flyout swaps the button's default action on each member action's `triggered`, which also covers hotkey activation.
