...
- Built imperatively in `MainWindow::build_tool_palette()` (main_window_actions_tool_palette.cpp, the tool-palette phase of `create_actions()`); order is the construction order, in separator-delimited clusters: select (Move, Marquee, Lasso, Wand), paint (Brush, Eraser, Gradient+Fill), retouch (Stamp, Healing, Smudge, Dodge), draw/type (Pen, Path Tools, Shapes, Type), view (Pick, Hand, Zoom). The Pen stays a standalone slot with the Path Tools flyout directly after it (drawing and adjusting paths alternate constantly, so the two slots stay adjacent); merging them into one flyout was considered and rejected because a flyout hides the tool you switch to most.
- Below the tools, an expanding spacer pins the color cluster to the bottom edge, ordered FG swatch, BG swatch, Default Colors (D), Swap Colors (X), separator, Quick Mask. That order is deliberate for vertical overflow: QToolBar hides items tail-first into the extension button, so a short window loses Quick Mask first, then X and D, and the swatches last. The palette width is a 43 px minimum, never fixed, because clicking the extension arrow expands the bar into extra columns by temporarily widening it; a fixed width clamps that into a clipped sliver (see the standing rule in [ui-conventions.md](ui-conventions.md)). The expansion is a toggle: stock Qt collapses it half a second after the pointer leaves the bar, so `ToolPaletteExpansionLock` swallows Leave while the extension button is checked, and the bar closes only on an arrow re-click or after picking any palette item (deferred one event-loop turn because the collapse hides the clicked button mid-release). Coverage: `ui_tool_palette_overflow_hides_quick_mask_before_swatches`, `ui_tool_palette_extension_button_expands_palette`, `ui_tool_palette_expanded_collapses_after_tool_pick`.
- Tools sharing a slot go through the shared `create_flyout_tool_action` + `configure_tool_flyout` lambdas (ten flyouts: marquee, lasso, wand, gradient+fill, stamp, healing [Healing Brush J / Spot Healing Shift+J / Patch unbound, default Healing Brush, see [healing.md](healing.md)], smudge/detail, dodge/tone, shape [Line/Rect/Ellipse/Polygon/Custom Shape, default Rect], path tools [Path Select A / Direct Select Shift+A]). The flyout swaps the button's default action on each member action's `triggered`, which also covers hotkey activation.
