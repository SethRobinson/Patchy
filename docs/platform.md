...
- `dialog_utils.cpp` `use_qt_file_dialog_controls` (Qt dialog widgets only under offscreen; native/portal dialogs otherwise, on every OS). The `QPrintDialog` behind the Print dialog's "Print Using System Dialog..." has no such switch: Qt 6 offers no widget fallback on Windows/macOS, so it is always the platform dialog and the UI test only checks the button, never clicks it.
