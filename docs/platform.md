...
- `print_dialog.cpp` `run_classic_windows_print_dialog` (Windows only): Windows 11 22H2+ swaps the `PrintDlgEx` dialog for a "unified" one that shows "This app doesn't support print preview" to every Win32 caller and fails its DEVMODE-to-print-ticket step for some drivers (0x80070002 with Microsoft Print to PDF). `QPrintDialog` always calls `PrintDlgEx`, so Patchy calls `PrintDlg` with a no-op `PD_ENABLEPRINTHOOK` hook (Microsoft's per-call opt-out) and feeds the result to Qt through the exported `QWin32PrintEngine::setGlobalDevMode`, whose declaration lives in a private header (`Qt6::PrintSupportPrivate`, linked only for WIN32 in CMakeLists). macOS and Linux keep `QPrintDialog`.
