...
- `skip_without_psd_text_face(layer, family)` adds the second way a machine lands off the Windows baseline: the family the imported layer actually resolved to. PSD PostScript names go through DirectWrite on Windows only; elsewhere the suffix-stripping heuristic in `psd_text_read.cpp` turns `Arial-Black` into family Arial plus the bold flag, which renders ~20% narrower **even where Arial Black is installed**. That is why the `snes-box-a3.psd` probes skip off Windows. Teaching that resolver to consult the font database off Windows is a real product fix and would restore the coverage.

## Remote suites with the fixture corpus present (August 2026, 0.87)

`local-test-fixtures/` is copied to studiomac and glados, so both remotes run the fixture-gated text tests. All four suites pass: both core suites, and the 927-test UI suite with no failures on each remote (a `[SKIP]` line reports everything a machine cannot cover: Linux prints 29, macOS 13, Windows 1). The core result includes `composite_corpus_flatten_digests_are_stable` against the **Windows-written** baselines: the CPU compositor is byte-identical across Windows, macOS arm64/clang, Linux x86-64/GCC, and wasm. Do not re-pin those baselines per machine.
