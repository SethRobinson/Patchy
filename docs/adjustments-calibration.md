...
- Editor: bounded four-channel histogram, one box-averaged sample per step-by-step block (`step = ceil(sqrt(pixels / 262144))`, ~262,000 samples; integer means over opaque pixels only, all-transparent blocks skipped). Averaging fills codes missing from quantized sources, matching PS's cache-pyramid histograms. The composite is the sum of the per-channel counts and the display is linear, normalized to the max bin, both matching PS; dialog Auto consumes the same averaged histograms, and the Levels dialog shares the sampler over the active layer. Re-editing an unclipped layer samples the layer-tree prefix below it (Auto sees the input); Auto is disabled for clipped adjustments. `.acv` Load accepts version 4 counted RGB, legacy version 1 bitmaps, and the indexed `Crv ` extension; Save writes PS's five-curve RGB shape with the trailing identity compatibility curve.
