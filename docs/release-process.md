...
- **Safe from any shell.** The `.bat` resets `PSModulePath` to the Windows PowerShell 5.1 set (the pwsh 7 problem in "Agent/non-interactive runs" below) and the `.ps1` hashes through .NET rather than `Get-FileHash`, so it also runs from pwsh 7 or the Claude terminal. The first v0.98 backfill was launched from pwsh 7 before either guard existed and published a `SHA256SUMS.txt` with empty hashes; the script now fails instead of writing a blank hash. To replace one asset on a public release by hand: `gh release upload v<version> <file> --clobber`.
