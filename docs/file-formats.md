...
PSD layer records keep their blending-ranges payloads opaquely in `Layer::raw_psd_blending_ranges()`. A folder's synthetic closing record has its own `raw_psd_group_boundary_blending_ranges()` payload so normal layers, adjustment layers, folder records, and group boundaries all re-emit the bytes they imported. These payloads carry Photoshop's Blend If settings; Patchy preserves them but does not render or edit them yet. Photoshop writes a 40-byte identity payload on ordinary records, so the compatibility report warns only when at least one 8-byte range differs from `00 00 ff ff 00 00 ff ff` (or the payload has an unknown shape). Fresh Patchy records keep the historical zero-length payload, which leaves the default writer canary unchanged.

Imported `lfx2`/`lrFX` blocks remain byte-identical until a user edits that layer's style. Satin and Pattern Overlay are parsed for preservation and regeneration, but Patchy does not render or edit them yet. The UI open path adds one localized import notice with recursive enabled-effect counts, the compatibility report warns per layer, and the Layer Style dialog repeats the warning because editing another effect regenerates the modeled descriptor and may normalize Photoshop-only options.
