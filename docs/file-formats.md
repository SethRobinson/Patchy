...
- **GIF** — write-only encoder gif_document_io.cpp: reading stays with the bundled qgif — so the Windows package must ship `imageformats/qgif.dll`; `scripts\release\build-release.bat`'s `CopyRequiredImageFormatPlugins` list includes it explicitly (macdeployqt and the Flatpak KDE runtime bundle it on the other platforms); LZW width-growth uses the pre-increment check, verified against Qt + Pillow, and `gif_encoder_bytes_are_stable` pins the exact bytes by FNV hash.
...
- Advanced Blending "Channels" reads and writes the `brst` layer block (channel blending restrictions): a bare list of big-endian u32 channel indices EXCLUDED from compositing, no count prefix (length/4 = count; empty = nothing restricted; Photoshop writes ascending indices and omits the block when everything blends). RGB documents with indices 0-2 map onto `Layer::restricted_channels()` (kRestrict* bits); the writer regenerates the block from the model (`should_skip_layer_block` drops the preserved copy, and an imported EMPTY block is dropped on resave like a default `iOpa`). Non-RGB indices or a non-multiple-of-four payload import as preserved raw only (`channel_restriction_supported() == false`, compatibility-report warning, dialog checkboxes disabled). Rendering semantics live in [ps-compat.md](ps-compat.md); pinned by `psd_channel_restrictions_round_trip`, `psd_channel_restrictions_unsupported_payload_preserved`, and the `photoshop-channel-restrictions.psd/bmp` fixture pair.
