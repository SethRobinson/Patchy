...
Serve the wasm build dir plus a harness page that shims `requestAnimationFrame` onto `setTimeout` AND re-implements the global timers on a Web Worker (Chrome throttles a hidden tab's main-thread timers to ~1/s; worker messages are exempt), suppresses `<a download>` clicks, passes `arguments: ['--stress-test=quick', '--stress-report-dir', '/stressout']` to qtLoad, and polls the report from MEMFS via the exported `FS`. The first run of a new binary (or port) pays V8 tier-up and is discarded as warm-up; configs are compared in interleaved pairs on two ports, never sequentially (machine-load drift beats most effects). The unattended `--run-script` mode works the same way (write the script into MEMFS from a `preRun` hook). Serving/build mechanics live in [wasm.md](wasm.md).
