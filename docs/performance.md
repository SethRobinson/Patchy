...
- The gate `moving_layers_should_use_outline_preview` measures WORK, not extent: the sum of every moving layer's canvas-clipped effect rect (averaged over the old and new deltas), against the same compile-time thresholds (4 Mpx, 1 Mpx styled). A dragged folder of overlapping stacked copies costs one composite per member per preview patch, so the per-layer sum predicts the frame cost while the old bounding-box metric stayed tiny (a 21-copy 1200x849 poster stack measured ~0.3 Mpx by box but ~460-575 ms per live frame). For disjoint layers the sum is at most the old bbox metric, so the stress contract calibration holds.
