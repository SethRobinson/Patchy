...
`Filter > Distort > Liquify...` opens a dedicated brush workspace. Liquify is not a `FilterRegistry` entry: it is an ordered sequence of gestures rather than one named invocation with stable parameters. It therefore does not appear in Visual Filters & Looks, cannot be saved in a Look, and has no private filter identifier. Its persisted hotkey command ID is `filter.liquify`; the default is Ctrl+Shift+X.
