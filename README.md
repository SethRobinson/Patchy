...
- The Paths panel now works like Photoshop's: a selected path stays visible on the canvas with any tool, drawing highlights the Work Path row, and selecting a shape layer shows its path outline until you dismiss it with Esc or a click on empty panel space. Ctrl-click a row to load it as a selection, drag rows to reorder saved paths, duplicate a path from the context menu, and turn any selection into a work path with the new Make Work Path from Selection command. Path thumbnails now show filled coverage so holes and boolean shapes read at a glance
