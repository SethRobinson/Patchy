...
- Smart Objects: place or convert layers to embedded or linked smart objects, edit or replace their contents, transform them non-destructively, and build editable native Gaussian Blur Smart Filter stacks with paintable shared masks and per-filter blending
...
- Editable Smart Filters currently support Gaussian Blur stacks, paintable shared masks, and per-filter opacity and blend modes. Unsupported imported filter types remain preview-locked and byte-preserved
